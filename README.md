...
<a href="https://muapi.ai?utm_source=github&utm_medium=readme&utm_campaign=awesome-claude-fable-5-1"><img src="images/banner.png" alt="Claude Fable 5.1 high-signal use case repository banner" width="760"></a>

[![License: MIT](https://img.shields.io/badge/License-MIT-lightgrey.svg)](LICENSE)
[![MuAPI support](https://img.shields.io/badge/MuAPI-Fable_5.1_coming_soon-111111)](https://muapi.ai?utm_source=github&utm_medium=readme&utm_campaign=awesome-claude-fable-5-1)
[![Official API](https://img.shields.io/badge/Official_API-claude--fable--5--1-cc785c)](https://platform.claude.com/docs/en/models/fable-5-1/whats-new-fable-5-1)
[![Model docs](https://img.shields.io/badge/Docs-Read-blue)](https://platform.claude.com/docs/en/models/fable-5-1/whats-new-fable-5-1)
[![Website](https://img.shields.io/badge/Website-Anthropic-orange)](https://www.anthropic.com/claude/fable)
...
**We collect real-world usage cases, tutorials, integrations, migration notes, and evaluations for Claude Fable 5.1, curated from first-party documentation, public demos, and creator communities.**

Claude Fable 5.1 launched on September 1, 2026. The current 5.1 section therefore prioritizes release evidence, API behavior, prompt patterns, benchmark results, and early production reports. The 94 entries in the baseline archive are explicitly labeled Claude Fable 5: they are useful predecessor evidence, but they are not silently relabeled as 5.1 evaluations.

Each current case links back to its original source. Community reports are labeled as such, while official capability and pricing claims link directly to Anthropic or Claude Platform documentation.

**MuAPI support for Claude Fable 5.1 is coming soon.** The model will be added to the MuAPI catalog when the route is live. Until then, use the official Claude API below; this README will be updated with the model-specific MuAPI playground and endpoint as soon as they are available.

| Surface | Identifier | Current details |
|---|---|---|
| **Claude API** | `claude-fable-5-1` | $10 / MTok input; $50 / MTok output |
| **Prompt-cache reads** | Fable 5.1 cache | $0.25 / MTok, 75% lower than Fable 5 |
| **Batch API** | Fable 5.1 batch | $5 / MTok input; $25 / MTok output |
| **AWS Bedrock** | `anthropic.claude-fable-5-1` | Available on Amazon Bedrock |
| **Google Cloud** | `claude-fable-5-1` | Available on Claude on Google Cloud |
| **MuAPI** | Coming soon | Model-specific route will be added when live |

> Pricing and availability can change. See the [official Fable 5.1 model documentation](https://platform.claude.com/docs/en/models/fable-5-1/whats-new-fable-5-1) for the current contract.

## 📊 Overview

- **10 current Claude Fable 5.1 cases** covering the launch, API migration, prompting, benchmarks, research, safeguards, and early production use.
- **94 Claude Fable 5 baseline cases** retained as a clearly labeled predecessor archive for transferable workflows and comparisons.
- Covers coding agents, long-running automation, games, visual design, 3D simulations, knowledge work, tutorials, API integrations, benchmarks, pricing, and practical limits.
- Use this repo to find evidence-backed workflows, compare the 5.1 upgrade path, discover reproducible prompt patterns, and track what is still early or unverified.

> [!NOTE]
> The collection favors concrete evidence over hype: reproducible prompts, shipped demos, benchmark methods, integration notes, cost data, and clearly stated caveats. Early 5.1 reports may be limited to a single account, surface, or partner evaluation.

> [!TIP]
> **MuAPI support is coming soon.** Follow [MuAPI](https://muapi.ai?utm_source=github&utm_medium=readme&utm_campaign=awesome-claude-fable-5-1) for the model-specific playground and pricing once Claude Fable 5.1 is added.

> 🚀 **Want to build your own AI SaaS with Fable 5.1?** [Awesome Generative AI Apps](https://github.com/Anil-matcha/awesome-generative-ai-apps) — 50+ production-ready AI SaaS templates with Stripe billing built in. Fork one, brand it, start charging.

## 🧭 Read First: Fable 5.1 in One Page

- **Model ID:** `claude-fable-5-1`; the model is available on the Claude API and partner platforms.
- **Context and output:** 1M-token context window and up to 128k output tokens.
- **Thinking:** adaptive thinking is always on; control depth with `effort`, not a legacy thinking budget.
- **Migration:** forced `tool_choice` values of `any` and `tool` are rejected; keep returned thinking blocks unchanged and keep conversation history append-only.
- **Safeguards:** refusals can return HTTP 200 with `stop_reason: "refusal"`; configure fallback handling for requests routed to Claude Opus 4.8 or Claude Opus 5.
- **Retention:** 30-day data retention applies by default; eligible enterprise customers may use zero-data-retention arrangements while Enterprise Frontier Safeguards roll out.

Read the [official release notes and migration guide](https://platform.claude.com/docs/en/models/fable-5-1/whats-new-fable-5-1) before changing a production agent.

<a id="-quick-api-access"></a>
## ⚡ Quick API Access

Use Claude Fable 5.1 through the official Claude API for now. MuAPI support is planned and will be documented here once the model-specific route is live.
...
| Section | Cases |
|---|---|
| [🆕 Claude Fable 5.1: Launch and Current Evidence](#fable-51-launch-and-current-evidence) | Case 95-104 |
| [📚 Claude Fable 5 Baseline Archive](#fable-5-baseline-archive) | Case 1-94 |
...
<a id="fable-5-baseline-archive"></a>
## 📚 Claude Fable 5 Baseline Archive

The following 94 entries are the original Claude Fable 5 collection carried forward from [awesome-claude-fable-5](https://github.com/Anil-matcha/awesome-claude-fable-5). They remain useful for transferable workflows, historical comparisons, and prompt ideas, but they should not be read as direct Fable 5.1 measurements.
...
<a id="fable-51-launch-and-current-evidence"></a>
## 🆕 Claude Fable 5.1: Launch and Current Evidence

These entries cover Claude Fable 5.1 itself. Because the model launched on September 1, 2026, the first edition emphasizes first-party release evidence, API migration, prompting, and carefully labeled early reports. The MuAPI route is intentionally marked as coming soon until it is live and verified.

## 🔌 MuAPI Integration Status

Claude Fable 5.1 will be added to MuAPI soon. Until the model-specific route is available, this repository links to the official Claude API for runnable examples and does not publish a guessed MuAPI endpoint or price. Once the integration is live, this section and the quick-access block above will be updated with the verified playground URL, request shape, pricing, and fallback behavior.

<a id="case-95"></a>
### Case 95: [Introducing Claude Fable 5.1 and Claude Mythos 5.1](https://www.anthropic.com/claude-fable-and-mythos-5-1) (by [Anthropic](https://www.anthropic.com/))

**Use the launch post as the primary source for what changed from Fable 5 and how the public Fable and trusted-access Mythos variants differ.**

Anthropic describes Fable 5.1 and Mythos 5.1 as the same underlying model with different safeguards. Fable 5.1 is generally available for coding and knowledge work; Mythos 5.1 is limited to trusted access programs for professional cybersecurity and life-sciences work.

The release focuses on stronger long-running coding, research, document, spreadsheet, slide, vision, and computer-use workflows. It also cuts cache-read pricing to $0.25 per million tokens, which Anthropic estimates will reduce typical token-billed workloads by about 25% and highly agentic workloads by up to approximately 45%.

Type: Integration | Date: 2026-09-01

---

<a id="case-96"></a>
### Case 96: [Official Benchmark Scorecard](https://www.anthropic.com/claude-fable-and-mythos-5-1) (by [Anthropic](https://www.anthropic.com/))

**Use the release scorecard to choose which of your own evaluations should receive a Fable 5.1 trial.**

Anthropic reports the following results with production safeguards enabled:

- Terminal-Bench-Science 0.1: **52.6%**.
- Terminal-Bench 4.0: **55.8%** for Fable 5.1, with **60.9%** reported for Mythos 5.1 on the same underlying model.
- GDPval-AA v2: **1853**.
- OSWorld 2.0: **77.9% partial** and **41.7% strict**.
- Humanity's Last Exam: **60.9% without tools** and **65.0% with tools**.
- AutomationBench: **31.4%**.
- CursorBench 3.2.0: **73.4%**.

The source warns that safeguard interventions can lower scores, that OSWorld uses an August 2026 task release, and that some results are not directly comparable with older benchmark runs. Treat these as launch evidence, then reproduce the relevant task mix with your own harness.

Type: Evaluation | Date: 2026-09-01

---

<a id="case-97"></a>
### Case 97: [Scientific Research Examples from the 5.1 Model Family](https://www.anthropic.com/claude-fable-and-mythos-5-1) (by [Anthropic](https://www.anthropic.com/))

**Use this case to separate public Fable 5.1 capabilities from limited-access Mythos 5.1 research demonstrations.**

Anthropic reports that Mythos 5.1 produced high-affinity protein binders validated by external organizations, with nearly a 50% hit rate across 12 targets and results substantially above prior designs on three targets. The post also describes Fable 5.1 training a higher-resolution elevation map of part of Venus from NASA Magellan radar data.

For computational biology, Anthropic says Mythos 5.1 wrote custom GPU kernels and cached intermediate results to speed seven open-source models by up to 2.5 times, with estimated genome-wide analysis cost reductions of 30% to 60%. The biology examples are not a promise of unrestricted public Fable access: Mythos 5.1 remains governed by trusted-access programs.
...
<a id="case-98"></a>
### Case 98: [Migrating an API Integration from Fable 5](https://platform.claude.com/docs/en/models/fable-5-1/whats-new-fable-5-1) (by [Claude Platform Docs](https://platform.claude.com/docs/))

**Use the migration checklist before swapping `claude-fable-5` for `claude-fable-5-1` in a production agent.**

The model ID changes to `claude-fable-5-1`, but the migration is not only a string replacement. Fable 5.1 rejects forced tool choices of `any` and `tool`; keep `tool_choice` at `auto` and use strict tool schemas or structured outputs when schema enforcement is needed.

Adaptive thinking remains always on. Pass thinking blocks back exactly as returned, keep the conversation history append-only, and avoid rebuilding earlier `system`, `tools`, or message prefixes. For new accounts, editing the prefix before a preserved thinking block can produce a 400 error or require an explicit block-dropping policy.
...
<a id="case-99"></a>
### Case 99: [Effort-Tuned Long-Horizon Work](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-fable-5-1) (by [Claude Platform Docs](https://platform.claude.com/docs/))

**Use effort as the first control for balancing quality, latency, and cost instead of running every task at maximum depth.**

The prompting guide recommends starting from the default effort and sweeping `low`, `medium`, `high`, `xhigh`, and `max` against your own eval set. Fable 5.1's gains over Fable 5 are reported across effort levels, while `medium` can roughly match the predecessor at lower cost on some workloads.

At `low`, the model is more likely to answer from memory and less likely to call search or retrieval tools, so raise effort or add an explicit verification instruction when freshness matters. For long coding and research tasks, reserve `high` or above for the steps where deeper reasoning changes the result.

Type: Tutorial | Date: 2026-09-01

---

<a id="case-100"></a>
### Case 100: [Progress Updates, Tool Batching, and Append-Only Agent Loops](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-fable-5-1) (by [Claude Platform Docs](https://platform.claude.com/docs/))

**Use the 5.1-specific loop guidance when an agent appears quiet, takes extra turns, or loses preserved reasoning.**

Fable 5.1 generally writes fewer user-facing updates during long tool chains. Clients that want visible status can opt into `thinking.display: "updates"` with the documented beta header and render non-empty thinking blocks as progress lines.

In implied coding and computer-use loops, independent reads may arrive one per turn. A useful prompt nudge is: “First privately list what you need next; then request every item that doesn't depend on another's result in this one response.” Keep assistant turns and thinking blocks byte-for-byte intact, and use turn-scoped system messages for temporary reminders instead of editing old history.
...
<a id="case-101"></a>
### Case 101: [Refusal, Fallback, and Billing Handling](https://platform.claude.com/docs/en/models/fable-5-1/whats-new-fable-5-1) (by [Claude Platform Docs](https://platform.claude.com/docs/))

**Treat safeguard behavior as part of the API contract when building a Fable 5.1 application.**

A declined request can return HTTP 200 with `stop_reason: "refusal"` and a `stop_details` object, so check the stop reason before indexing into the content array. Fable 5.1 can use server-side fallback, SDK middleware, or a manual retry; the documented fallback targets are Claude Opus 4.8 and Claude Opus 5.

Requests refused before any output are not billed, and fallback credit can refund the prompt-cache cost of switching models. Fable 5.1 is more permissive for defensive vulnerability identification, but penetration testing, exploit generation, and binary-based vulnerability scanning can still be redirected to an Opus model.
...
<a id="case-102"></a>
### Case 102: [Cache Economics and Data-Retention Planning](https://platform.claude.com/docs/en/models/fable-5-1/whats-new-fable-5-1) (by [Claude Platform Docs](https://platform.claude.com/docs/))

**Use the new cache-read price and retention rules to model the real cost of long-running agents before launch.**

Fable 5.1 keeps $10 per million input tokens and $50 per million output tokens. Five-minute cache writes remain $12.50 per million, one-hour cache writes remain $20 per million, and cache reads fall to **$0.25 per million tokens**. Batch processing is listed at $5 per million input tokens and $25 per million output tokens.

The model carries 30-day data retention by default. Anthropic's Enterprise Frontier Safeguards are rolling out in phases to let eligible enterprise customers keep data in customer-controlled cloud infrastructure with a zero-data-retention experience; confirm eligibility and rollout status before putting sensitive workloads on the API.
...
<a id="case-103"></a>
### Case 103: [Early Partner Evidence on Coding and Knowledge Work](https://www.anthropic.com/claude-fable-and-mythos-5-1) (by [Anthropic](https://www.anthropic.com/))

**Use the partner reports as hypotheses for your own evals, not as independent benchmark results.**

The launch post summarizes early testing from companies including Jane Street, Cognition, MongoDB, Red Hat, Browserbase, Datadog, Canva, Shopify, and others. Their reported patterns include stronger long-session code review, more reliable root-cause analysis, unattended prototypes with verification loops, better browser-agent completion, denser research and slide work, and lower token use on some workloads.

These are customer statements selected by the vendor, so they are directional evidence rather than neutral measurements. The practical takeaway is to evaluate Fable 5.1 on the work that is expensive to supervise: multi-service changes, rare production failures, long research chains, and deliverables that need the model to inspect its own output.
...
<a id="case-104"></a>
### Case 104: [Early Claude Code Rollout Signals](https://www.reddit.com/r/ClaudeCode/comments/1w4k4gf/fable_51_just_showed_up_in_my_model_picker/) (by [Claude Code community](https://www.reddit.com/r/ClaudeCode/))

**Use community reports to track surface-level rollout signals while keeping official documentation as the source of truth.**

On September 1, community members reported seeing Fable 5.1 in the Claude Code model picker, with a separate report noting that the model was visible in the mobile app. These reports are useful for identifying which product surfaces may be rolling out first, but they do not establish universal access, subscription limits, API availability, or partner-platform support.

For rollout checks, record the account type, client version, region, model label, and API behavior. Re-check against the [official Fable 5.1 model documentation](https://platform.claude.com/docs/en/models/fable-5-1/whats-new-fable-5-1) before making a production or pricing decision.
...
This repository was inspired by the creators, developers, benchmark teams, and communities who shared Claude Fable 5 and Claude Fable 5.1 usage cases publicly.
...
If you have more interesting usage cases to share, feel free to open an issue or pull request and help expand the Fable 5.1 and MuAPI use case library. Please identify the exact model, surface, date, and evidence type when submitting a case.

[![Star History Chart](https://api.star-history.com/svg?repos=Anil-matcha/awesome-claude-fable-5-1&type=Date)](https://www.star-history.com/#Anil-matcha/awesome-claude-fable-5-1&Date)
